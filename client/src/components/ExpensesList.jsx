...
const ExpensesList = ({ expenses, onEdit, onDelete }) => {
  ...
  return (
    <div className="bg-zinc-900/80 backdrop-blur-lg rounded-2xl shadow-2xl p-6 mb-6 border-2 border-blue-900 text-white animate-fadein w-full overflow-hidden">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-blue-900">
                ...
                <th className="text-left py-2 px-2">Split Type</th>
            ...
            <tbody>
              {safeExpenses.map((expense, index) => (
                <tr key={expense._id || index} className="border-b border-zinc-800 hover:bg-zinc-800/50 transition-all duration-200">
                  ...
                  <td className="py-2 px-2">
                    <div className="max-w-24 truncate" title={expense.paid_by}>
                      {expense.paid_by}
                    </div>
                  </td>
                  <td className="py-2 px-2">
                  ...
                  <td className="py-2 px-2">
                    <div className="flex gap-2 justify-center">
                      <button
                        ...
                        className="bg-blue-600 hover:bg-blue-700 text-white px-2 py-1 rounded text-xs font-bold transition-all duration-200"
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => onDelete(expense._id)}
                        className="bg-red-600 hover:bg-red-700 text-white px-2 py-1 rounded text-xs font-bold transition-all duration-200"
                      >
                        Delete
                      </button>
                    </div>
