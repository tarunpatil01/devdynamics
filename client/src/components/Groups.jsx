...
const Groups = ({ group, people, onAddPerson, messages, onSendMessage }) => {
  ...
  return (
    <div className="flex flex-col h-full w-full max-w-lg mx-auto p-2 md:p-4 bg-black text-white">
      <h2 className="text-2xl font-bold text-white mb-4">Group: {group?.name || 'Select a group'}</h2>
      <div className="mb-4">
          ...
          {people && people.length > 0 ? people.map((person, idx) => (
            <li key={person._id || person.name || idx} className="flex items-center gap-2 py-1 px-2 bg-gray-900 rounded shadow text-white">
              {person.avatar ? (
                ...
                <span className="w-6 h-6 flex items-center justify-center rounded-full bg-gray-700 text-white font-bold text-xs">
          ...
          {messages && messages.length > 0 ? messages.map((msg, idx) => (
            <div key={msg._id || idx} className="mb-2 flex items-start gap-2">
              {msg.sender && msg.sender.avatar ? (
                ...
                <span className="w-6 h-6 flex items-center justify-center rounded-full bg-gray-700 text-white font-bold text-xs mt-1">
