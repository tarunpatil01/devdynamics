...
const Sidebar = ({ showGroups, setShowGroups, sidebarOpen, setSidebarOpen }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 z-40 h-screen w-80 sm:w-72 bg-zinc-900/95 backdrop-blur-lg shadow-2xl border-r-2 border-blue-900 rounded-r-2xl flex flex-col transition-transform duration-300 ease-in-out sidebar-mobile md:sidebar-desktop
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 md:p-6 border-b border-blue-900">
          <h2 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-white drop-shadow">Split App</h2>
          <button 
            className="text-white md:hidden touch-target p-2 hover:bg-zinc-800 rounded-lg transition-colors" 
            onClick={() => setSidebarOpen(false)} 
        ...
        <nav className="flex-1 flex flex-col gap-3 md:gap-4 p-4 md:p-6 overflow-y-auto">
          <button
            className={`w-full text-left px-4 md:px-6 py-4 md:py-5 rounded-2xl font-semibold transition-all duration-200 text-base md:text-lg flex items-center gap-3 shadow-lg touch-target ${!showGroups ? 'bg-blue-700 text-white' : 'bg-zinc-800 text-blue-300 hover:bg-zinc-700'}`}
            onClick={() => { setShowGroups(false); setSidebarOpen(false); }}
          >
            <span role="img" aria-label="split" className="text-xl md:text-2xl">💸</span> Split
          </button>
          <button
            className={`w-full text-left px-4 md:px-6 py-4 md:py-5 rounded-2xl font-semibold transition-all duration-200 text-base md:text-lg flex items-center gap-3 shadow-lg touch-target ${showGroups ? 'bg-blue-900 text-blue-200' : 'bg-zinc-800 text-purple-300 hover:bg-zinc-700'}`}
